...
class TileWorker(Process):
    ...
    def _get_dz(self, associated=None):
        if associated is not None:
            ...
            image = self._slide
        return DeepZoomGenerator(image,
                                 self._tile_size,
                                 self._overlap,
            ...
            for row in range(rows):
                for col in range(cols):
                    ...
                    if not os.path.exists(tilename):
                        self._queue.put(
                            (self._associated, level, (col, row), tilename))
                    self._tile_done()
...
if __name__ == '__main__':
    ...
    DeepZoomStaticTiler(slidepath, opts.basename, opts.format, opts.tile_size,
                        opts.overlap, opts.limit_bounds, opts.quality,
